...
--Ckecking game Gravity Rush
GRTB = {
	{ id = "PCSA00011", region = "North America" },
	{ id = "PCSF00024", region = "Europe" },
	{ id = "PCSD00035", region = "Asia" },
	{ id = "PCSD00003", region = "Chinese version" },
}

gamesGR = {}
for i=1,#GRTB do
	if game.exists(GRTB[i].id) then
		GravityRush = true
		table.insert(gamesGR,GRTB[i])
	end
end

function GR544P()

	if not GravityRush then os.dialog(LANGUAGE["NO_GRAVITYRUSH_GAMES"], LANGUAGE["MENU_PSVITA_GR"]) return end

	local GravityRush_ID = "GR544P.png"

	local patches = {
		{ res = LANGUAGE["MENU_PSVITA_GR"],  desc = LANGUAGE["INSTALLP_DESC_GR544P"], path = "GR544P.suprx" },
	}

	for i=1,#patches do
		for j=1,#plugins do
			if patches[i].desc == plugins[j].desc then
				patches[i].res = plugins[j].name .. " " .. plugins[j].v
			end
		end
	end
	...
	while true do
		...
		screen.print(480,20,LANGUAGE["MENU_PSVITA_GR"],1.0,color.white,color.blue,__ACENTER)

		draw.fillrect(0,64,960,322,color.shine:a(25))

		--Games
		local xRoot = 200
		local w = (955-xRoot)/#gamesGR
		for i=1, #gamesGR do
			if selector == i then
				draw.fillrect(xRoot,63,w,42, color.green:a(90))
			end
			screen.print(xRoot+(w/2), 75, gamesGR[i].id, 1, color.white, color.blue, __ACENTER)
			xRoot += w
		end
		draw.fillrect(750,435,200,42, color.shine:a(25))
		screen.print(940,445,gamesGR[selector].region,1,color.white,color.blue, __ARIGHT)
		...
		for i=scroll.ini, scroll.lim do
			...
			idx = tai.find(gamesGR[selector].id,patches[i].path)
			if idx != nil then
				if files.exists(tai.gameid[ gamesGR[selector].id ].prx[idx].path) then
		...
		if scroll.maxim > 0 then
			...
			if buttons.released.l or buttons.released.r then
				...
				if selector > #gamesGR then selector = 1
				elseif selector < 1 then selector = #gamesGR end
			end
			...
			if buttons.accept then
				...
				local img = image.load(screenshots..GravityRush_ID)
				if not img then
					if http.download(string.format("https://raw.githubusercontent.com/%s/%s/master/screenshots/%s", APP_REPO, APP_PROJECT, GravityRush_ID), screenshots..GravityRush_ID).success then
						img = image.load(screenshots..GravityRush_ID)
					end
				end
				...
				Patch_GravityRush_install(gamesGR[selector],patches[scroll.sel])
			end
...
function Patch_GravityRush_install(game,res)
